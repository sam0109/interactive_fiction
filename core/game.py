...
class Character:
    def __init__(self, name, context, description=None, inventory=None):
        ...
        self.history = []
        self.inventory = (
            inventory if inventory is not None else {"money": 0, "items": {}}
        )
...
class Game:
    ...
    def transfer_money(self, sender_name, recipient_name, amount):
        ...
        if sender.inventory["money"] < amount:
            return (
                f"({sender.name} tries to give {amount} gold but doesn't have enough)"
            )
        ...
        print(f"[Action] {sender.name} gave {amount} gold to {recipient_display_name}")
    ...
    def transfer_item(self, sender_name, recipient_name, item_name):
        ...
        print(f"[Action] {sender.name} gave {item_name} to {recipient_display_name}")
            ...
            try:
                ...
                if 0 <= char_index < len(self.characters):
                    ...
                    if self.current_character.description:
                        ...
                        safe_name = "".join(
                            c
                            for c in self.current_character.name
        ...
        while True:
            ...
            while True:
                ...
                if len(self.current_character.history) > config.MAX_HISTORY:
                    self.current_character.history = self.current_character.history[
                        -config.MAX_HISTORY :
                    ]
